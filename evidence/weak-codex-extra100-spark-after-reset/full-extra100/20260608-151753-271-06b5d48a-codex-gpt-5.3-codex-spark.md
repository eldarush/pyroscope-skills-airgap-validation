...
## stdout
case_id=mixed_ext_raw_pyroscope_json_request
route=analyze
runtime=mixed
...
session id: 019ea72b-032f-7e20-b6be-8e0c167c0ec3
--------
user
...
Case id: mixed_ext_raw_pyroscope_json_request
Runtime: mixed
Difficulty: hard
Task: User provides raw unbounded Pyroscope JSON and asks the weak model to infer optimizations directly.

Strong evaluator feedback from previous round:
Fix these exact issues: decision: expected plan-only-for-unsafe, got refuse. Return only the required key=value lines.
